...
class TestBlasdot(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.test_matrices = [
            (np.random.rand(1, 1), np.random.rand(1, 1)),
            (np.random.rand(1, 5), np.random.rand(5, 1)),
            (np.random.rand(5, 1), np.random.rand(1, 5)),
            (np.random.rand(5, 5), np.random.rand(5, 5)),
            (np.random.rand(4, 5), np.random.rand(5, 4)),
            (np.random.rand(5, 4), np.random.rand(4, 5))
        ]
        self.test_matrices += \
            [(a.copy(order='F'), b) for a, b in self.test_matrices] + \
            [(a, b.copy(order='F')) for a, b in self.test_matrices] + \
            [(a.copy(order='F'), b.copy(order='F')) for a, b in self.test_matrices]
