...
class TestBlasdot(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.test_matrices = [
            (np.random.rand(2, 3), np.random.rand(3, 4)),
            (np.array(np.random.rand(4, 5), order='f'), np.array(np.random.rand(5, 2))),
            (np.array(np.random.rand(3, 5)), np.array(np.random.rand(5, 4), order='f')),
            (np.array(np.random.rand(5, 3), order='f'), np.array(np.random.rand(3, 6), order='f'))
        ]
        self.test_matrices += [(b.T, a.T) for a, b in self.test_matrices]
        self.test_matrices += [(np.array(a, dtype=np.float32), np.array(b, dtype=np.float32))
