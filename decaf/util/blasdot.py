...
def gemm_f_contiguous(alpha, A, B, out=None):
    ...
    elif out.shape != (A.shape[0], B.shape[1]) or out.dtype != A.dtype or not out.flags.f_contiguous:
        raise ValueError('Incorrect output data type')
    if A.flags.c_contiguous and B.flags.c_contiguous:
        scipy_gemm(alpha, a=A.T, b=B.T, trans_a=True, trans_b=True, c=out, overwrite_c=True)
